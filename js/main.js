...
function initGame() {
    ...
    c.addToCanvas(baddy, 0, 0);  // Send the element to canvas, which will put it in the corresponding coordinates
    ...
    c.addToCanvas(baddy2, 10, 0);

    c.movePlayerNorth(1);  // Move the player North by 1 cell
    c.moveNorth("player", 1);  // This is equivalent to the above call
    c.moveWest("baddy-1",1);  // Move baddy-1 one unit west
    c.moveSouth("baddy-2",10);  // Move baddy-2 south 10 units
    c.updateImage("player", "img/letter_c.jpg")  // You can update the graphic of any element on the canvas,
